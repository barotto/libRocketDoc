...
The single-line text field control is specified in RML by the `<input type="text" />` tag. A password-style text field can be specified by the `<input type="password" />` tag. The interface to both of these elements is the {{page.lib_ns}}::Controls::ElementFormControlInput class.
...
The text area, or multi-line text field, is specified in RML with the `<textarea>` tag. Any loose text between the text area's opening and closing tag will become the initial value of the control. The interface to the text area is the {{page.lib_ns}}::Controls::ElementFormControlTextArea class.
...
The data-driven drop-down select control is specified in RML with the <dataselect> tag. No options are specified within the tag; instead, they are populated from an {{page.lib_ns}}::Core::DataSource object, similarly to a data grid.

The select control's interface is the {{page.lib_ns}}::Controls::ElementFormControlDataSelect class. It derives from {{page.lib_ns}}::Controls::ElementFormControlSelect. The data select's data source is set with the "source" attribute, and can be changed in C++ through the SetDataSource() method:
...
The range control can be used to render a slider-based number field. It is specified in RML with the tag `<input type='range' />`. The range control's interface is the {{page.lib_ns}}::Controls::ElementFormControlInput class.
...
The form submit button is specified in RML with the `<input type="submit" />` tag. The submit button will trigger a submit on its ancestor form when it is clicked, with a submit value equal to its "value" attribute. Its interface is the class {{page.lib_ns}}::Controls::ElementFormControlInput. 
